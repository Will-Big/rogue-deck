...
21. **그래프는 커밋되지 않는 로컬 산출물이다. 쓰기 전에 새로 만든다.** `graph.json`·`manifest.json`·
    ...
    갱신을 대신해 주는 훅은 없다 — 과거의 post-commit/post-merge 파이프라인은 폐기됐다(2026-08-28 확인,
    활성 훅 0개). 그래프 최신성은 오직 위의 삭제 후 재생성으로 보장한다.
...
28. **설계·계획 문서는 한 파일 안에서 사람 검수 절과 세션 인계 절로 나눈다.** 지금까지 문서의 주
    ...
    개입 카드에서 확장형으로 세 번 겪었다. 새 스펙·계획은 다음 골격을 쓴다.
    ...
    사람 절은 **60줄 이내**이고 코드 블록·필드명 나열·알고리즘 설명을 넣지 않는다. "어떤 로직인가"가
    아니라 "무엇이 무엇에 의존하는가"만 쓴다.

    - **마지막 두 항목은 비워 둘 수 없다.** SO의 런타임 생성 한계처럼, 알고 있었어야 할 제약이
...
29. **개요로 승인받고, 상세로 구현한다.**
    - 문서를 참조할 때 **실행 근거는 `## 상세`에서만** 얻는다. `## 설계 개요`는 방향 확인용이며,
      개요만 읽고 구현에 들어가지 않는다.
    - 새 스펙·계획을 만들면 **`## 설계 개요`를 제시해 승인받은 뒤** 상세를 실행한다. 상세 전문을
      붙여 승인을 요청하지 않는다.
