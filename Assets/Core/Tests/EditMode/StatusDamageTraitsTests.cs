...
namespace FateWeaver.Tests
{
    ...
    /// (계획 D10). 독처럼 적은 상태만 방어를 뚫고, 적지 않은 상태 피해는 공격처럼 방어·취약을 거친다.</summary>
    public class StatusDamageTraitsTests
    {
        ...
        private static StatusContentCatalog Catalog(DamageTraitsSpec burnDamage)
        ...
        {
            var state = new CombatState(Catalog(null));
        ...
        {
            var state = new CombatState(Catalog(new DamageTraitsSpec { Piercing = true, IgnoresMultipliers = true }));
        ...
        public void Damage_traits_round_trip_through_status_json()
        {
            var spec = new PoisonStatusSpec
            {
                ...
                GrowthPerTurn = 1, Damage = new DamageTraitsSpec { Piercing = true }
            };
            ...
            StringAssert.Contains("\"piercing\": true", json);
            StringAssert.DoesNotContain("ignoresMultipliers", json, "false는 생략한다");
            Assert.AreEqual(new DamageTraits(true, false), restored.Damage.ToTraits());
        }
    }
}
