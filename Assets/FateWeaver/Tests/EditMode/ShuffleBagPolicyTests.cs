...
namespace FateWeaver.Tests
{
    public class ShuffleBagPolicyTests
    {
        ...
        private static string Signature(ShuffleBagPolicy policy, int turns)
        ...
        {
            var policy = new ShuffleBagPolicy(Catalog("a", "b", "c", "d", "e", "f"), 2, seed: 11);
            var firstCycle = Enumerable.Range(0, 3)
                .SelectMany(t => policy.CardsForTurn(t).Select(c => c.Id))
                .ToArray();

            CollectionAssert.AreEquivalent(new[] { "a", "b", "c", "d", "e", "f" }, firstCycle);
            Assert.AreEqual(6, firstCycle.Distinct().Count());
            Assert.AreEqual(2, policy.CardsForTurn(3).Count);
        }
        ...
        {
            Assert.AreEqual(
                Signature(new ShuffleBagPolicy(Catalog("a", "b", "c", "d"), 2, seed: 7), 6),
                Signature(new ShuffleBagPolicy(Catalog("a", "b", "c", "d"), 2, seed: 7), 6));
            Assert.AreNotEqual(
                Signature(new ShuffleBagPolicy(Catalog("a", "b", "c", "d"), 2, seed: 7), 6),
                Signature(new ShuffleBagPolicy(Catalog("a", "b", "c", "d"), 2, seed: 8), 6));
        }

        [Test]
        public void Reshuffles_full_deck_when_remaining_cards_are_insufficient()
        {
            var policy = new ShuffleBagPolicy(Catalog("a", "b", "c", "d", "e"), 3, seed: 3);
            var first = policy.CardsForTurn(0).Select(c => c.Id).ToArray();
            var second = policy.CardsForTurn(1).Select(c => c.Id).ToArray();
        ...
        {
            Assert.AreEqual(0, new ShuffleBagPolicy(Array.Empty<CardDefinition>(), 2, seed: 1).CardsForTurn(0).Count);
            Assert.AreEqual(0, new ShuffleBagPolicy(Catalog("a", "b"), 0, seed: 1).CardsForTurn(0).Count);
            Assert.AreEqual(0, new ShuffleBagPolicy(Catalog("a", "b"), -2, seed: 1).CardsForTurn(0).Count);
        }
    }
}
