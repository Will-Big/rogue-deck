...
2026-09-04 실측이다. 워크트리에만 헤드리스 에디터를 띄운 상태에서 `unity mcp`를 **메인 체크아웃을
cwd로** 띄우고 `editor_status`를 부르니, 도구 149개가 그대로 나오고 `projectPath`가
`.claude/worktrees/unity-mcp-setup`으로 돌아왔다. 상위 문서의 "cwd를 포함하는 프로젝트의 에디터를
고른다"는 서술은 **에디터가 여럿일 때 고르는 규칙**이지, 하나일 때의 안전장치가 아니다.

**여기가 규칙 15와 부딪히는 지점이다.** 평소 열려 있는 에디터는 메인 체크아웃
(`/Users/ish/Git/rogue-deck`)의 것 하나뿐이다. 그러면 위 실측의 방향이 뒤집혀
**워크트리 세션이 메인 체크아웃의 에디터를 잡는다** — 워크트리에 갇혀 있다고 믿는 세션이 사용자
전용 체크아웃의 씬·프리팹·에셋을 고치게 되고, `git status`에는 아무것도 안 뜬다. 워크트리의
`git status`를 보고 있기 때문이다. (이 방향은 메인 체크아웃에 에디터를 띄워야 재현되므로 실측하지
않았다. 확인된 것은 대칭인 반대 방향이며, cwd가 대상 선택에 관여하지 않는다는 사실 자체다.)
