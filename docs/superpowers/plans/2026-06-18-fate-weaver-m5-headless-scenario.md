...
Create a pure-C# simulation layer that builds a `CombatState` from scenario data, applies scripted fate plays, resolves the turn, and emits a readable Markdown report.
...
- Use existing Core handlers: damage, reward-nullify, change initiative, swap initiative, lock.
...
- [x] Add scenario data types: enemies, zone cards, fate plays.
- [x] Add `ScenarioRunner`.
- [x] Add `ScenarioReport.ToMarkdown`.
- [x] Add a sample scenario where `SwapInitiative` turns `quick_cut` from `Basic` into `Success`.
...
3. RED: assert Markdown report contains initial order, fate play summary, resolution, and final state.
