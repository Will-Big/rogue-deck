...
> **2026-09-23 개정 — 카드 머리 행.** 손패를 참고 이미지 수준(약 36%)으로 겹치자 오른쪽 위 비용이
> 다음 카드에 통째로 가려졌다(§7 검토 초점 5번이 예고하고 §5 작업 4가 캡처 검토를 요구한 경우). 사용자가 선택지 중 "비용을 왼쪽으로"를 골랐고
> 캡처로 검수한다. 머리 행은 `[순서·종류 탭 10–40][비용 44–72][이름 76–188, 왼쪽 정렬]`이다.
> §3의 "이름은 카드 내부 상단 가운데, 비용은 내부 오른쪽"은 이 개정으로 대체한다. 비용과 이름의 세로
> 중심 일치, 이름 폭 112, 두 줄 제한은 유지한다. 손패 계약은
> [프레임 스펙 §13](../specs/2026-07-31-primitive-card-frame-design.md)에 있다.
