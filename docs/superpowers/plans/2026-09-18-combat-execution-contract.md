...
- 기준 수치(T4 끝): 헤드리스 712 · 편집 도구 157 · Unity EditMode 905(통과 898, 실패 0, 건너뜀 7 = `CardFrameRenderCapture`).
...
| D10 | **관통·배율 미적용은 피해의 공통 속성이며 데이터에 둔다.** 독만의 성질이 아니고 다른 피해도 쓸 수 있다. 상태 피해라는 원인만으로 관통하지 않는다 — 독은 `poison.json`의 `damage`로 관통·배율 미적용이다(T4 검토, 2026-09-18) | T4 |
| D11 | **연쇄 금지는 반응 공격이 반응 공격을 부르지 않는 것이다.** 사망 시 반응(전염)은 반응 공격과 다르며 사망 원인이 반응이어도 발동한다. 계획 초안의 "반격 사망과 전염 → 전염 미발동"을 뒤집는다(T4 검토, 2026-09-18) | T4 |
...
- **연쇄 금지(D11)**: 반응 능력마다 `RespondsToReactionEvents`를 밝힌다. Primary 사건에는 모든 능력이, 반응 기원 사건에는
  이 값이 true인 능력만 반응한다(`ReactionDispatcher.Allows`). 반격 같은 반응 공격은 false, 사망 시 반응(전염)은 true —
  반격으로 죽어도 전염이 발동한다. true는 같은 사건이 되풀이될 수 없는 능력에만 쓴다(보유자는 한 번만 죽는다).
...
- **공통 피해 경로**: `DamageService`(원인 공격/상태 + 피해 속성 `DamageTraits`). 카드 피해·반응 피해는 공격, 상태 틱·즉시 발동은
  상태 원인이다. **관통·배율 미적용은 원인이 아니라 데이터가 정한다(D10)**: 상태의 `damage`(`StatusSpec.Damage` →
  `StatusContentCatalog.DamageTraitsOf`)를 읽고, 생략하면 보통 피해(방어·배율 적용)다. 독은 `poison.json`에 둘 다 적었다(D1). 독의 `StatusTicked`는 이제 피해 **전에** 기록한다. 공통 경로가
...
- T4 검토 후 수정(D10·D11, 별도 커밋): 헤드리스 712(피해 속성 테스트 `StatusDamageTraitsTests`, 반격-반격 금지·사망 시 반응 테스트 추가).
...
| 반격에 대한 반격 | 반응 공격은 반응 공격을 부르지 않음(D11) |
