...
- 교환은 실행 순서만 바꾼다 — 교환된 두 카드는 자리의 진영까지 물려받고, 뒤에 오는 카드는 교환을 모르는 것처럼
  자리를 찾는다(스펙 §6 개정).
- 주인 사망으로 빠진 카드는 새 이벤트 `CardRemoved`로 기록한다(`CardCancelled`가 아니다).
- C# 샘플 `CounterStance`를 지웠다(효과마다 조건이 달라 카드당 단일 시작 조건으로 옮길 수 없었다).
- 편집 도구의 결과 참조 칸은 앞에 소비 효과가 있을 때만 한 줄로 보인다.

이 세션에서 겪은 함정:
- **고정 서명(`GoblinParityTests`)이 깨지면** 이전 커밋 기준 복사본을 만들어 서명 텍스트를 비교하고, 의도한 차이만큼만
  갱신하며 사유를 주석에 쓴다. 방법: `git archive -o <scratch>/head.tar HEAD Assets/Core Assets/StreamingAssets` →
  scratch 복사본에 풀기 → 기대값을 틀린 값으로 바꿔 실패 메시지의 "실측 서명"을 뽑는다(`dotnet test … -p:TargetFramework=net5.0
  --filter FullyQualifiedName~Node_zero_signature`). T1·T2a에서 이렇게 한 줄씩 갱신했다.
- **`Tools/card-idea-notebook/`를 건드리면 `pre-commit`이 편집 도구 테스트를 돌린다**(`.githooks/pre-commit` 102줄).
  C# 저작 타입을 바꾸면 스키마가 재생성되므로 편집 도구도 같은 커밋에서 맞춰야 한다.
- **Unity 배치는 워크트리 대상으로 돌린다**(D3). 첫 실행은 임포트로 오래 걸린다. 결과 XML의 `failed`를 본다.
  Unity 테스트(`Assets/Tests/UnityEditMode`)가 코어 생성자를 위치 인자로 부르는 곳이 있어, 이름 grep으로는 안 잡힌다(T0).
...
| D10 | **관통·배율 무시는 피해 속성이며 데이터에 둔다. 한 피해가 여러 속성을 가질 수 있다.** 독만의 성질이 아니다. 카드 피해 효과는 `traits`, 상태는 `damageTraits` 목록으로 적고, 생략하면 보통 피해다. 편집 도구에 여러 개 고르는 필드(`enumSet`)를 연다(T4 검토, 2026-09-18) | T4 |
| D11 | **반응을 한 묶음으로 판정하지 않는다.** 사건 종류마다 조건이 따로다: 공격에 대한 반응은 반응 공격이 낸 공격에 발동하지 않고, 사망 시 반응(전염)은 사망 원인과 무관하게 발동한다. 방어 획득 반응도 반응이 준 방어에 발동하며, 무한 반복을 피하는 것은 능력 설계의 책임이다. 계획 초안의 "반격 사망 → 전염 미발동"·"반응의 방어 획득 → 획득 반응 미연쇄"를 뒤집는다(T4 검토, 2026-09-18) | T4 |
...
- **반응 판정(D11)**: 기원으로 사건을 일괄 막는 규칙이 없다. 모든 사건(반응 효과가 낸 것 포함)이 디스패처로 가고, 사건에는
  그것을 낸 효과의 기원(`CombatSignal.Origin`)이 실린다. 발동 여부는 각 능력의 `CanReact`가 사건 종류별로 정한다: 공격에
  대한 반응은 `Origin == Reaction`인 공격에 발동하지 않고(반격이 반격을 부르지 않는다), 사망 시 반응(전염)은 기원을 보지
  않는다. 방어 획득 반응은 반응이 준 방어에도 발동한다. 무한 반복을 만드는 능력을 피하는 것은 능력 설계의 책임이다.
...
- **공통 피해 경로**: `DamageService`(원인 공격/상태 + 피해 속성 `DamageTraits`). 카드 피해·반응 피해는 공격, 상태 틱·즉시 발동은
  상태 원인이다. **관통·배율 무시는 원인이 아니라 데이터가 정한다(D10)**: 속성은 `DamageTrait` 목록이다. 카드 피해는
  `DamageSpec.Traits`(JSON `traits`) → `DamagePayload`, 상태는 `StatusSpec.DamageTraits`(JSON `damageTraits`) →
  `StatusContentCatalog.DamageTraitsOf`. 생략하면 보통 피해다. 독은 `poison.json`에 둘 다 적었다(D1). 빈 목록은 파일에 쓰지
  않는다(`ShouldSerialize`, 편집 도구도 같다). 카드 설명은 피해 뒤에 " (관통, 배율 무시)"를 붙인다.
- **편집 도구**: 스키마 필드 타입 `enumSet`(배열 열거형, 체크박스)을 더했다. 스키마 내보내기의 순서 탐침은 조건부 생략
  (`ShouldSerialize`)을 끄고 필드를 읽는다 — 켜 두면 필드가 스키마에서 조용히 빠진다. 독의 `StatusTicked`는 이제 피해 **전에** 기록한다. 공통 경로가
...
- T4 검토 후 수정(D10·D11, 커밋 두 개): 헤드리스 715 · 편집 도구 161 · Unity EditMode 908(통과 901, 실패 0, 건너뜀 7).
  피해 속성 테스트 `StatusDamageTraitsTests`(상태·카드), 반격-반격 금지, 사망 시 반응, 반응 방어 획득의 연쇄, 편집 도구 enumSet 테스트를 더했다.
  테스트 전용 상태로 턴 시점 처리를 검증하는 테스트는 `TestContent.PlainStatuses(...)` 카탈로그를 쓴다 — 코어는 등록된
  상태마다 저작 콘텐츠(피해 속성 포함)가 있다고 가정한다. 고정 서명·설명·Compare는 여전히 같다.
...
- DamageService는 피해 원인(공격/상태), Piercing(방어 흡수 우회), 배율 적용 여부를 따로 받는다.
  **독은 원인=상태, Piercing, 배율 미적용으로 호출한다(D1). 이 두 속성은 poison.json 데이터가 정한다(D10).** 지금 독은 방어·배율을 모두 거치지 않으며
...
| 반격 사망과 전염 | 사망·카드 제거는 실행, 전염도 발동(D11로 개정) |
| 반격에 대한 반격 | 공격에 대한 반응은 반응 공격에 발동하지 않음(D11) |
| 피해 속성을 저작하지 않은 상태 피해 (D10) | 방어에 흡수되고 배율을 받음 |
| 반응 효과의 방어 획득 | 방어는 증가, 방어 획득 반응도 발동(D11로 개정) |
