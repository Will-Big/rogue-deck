...
- 작성일: 2026-07-16
- 상태: 후속 설계/구현 대기
- 현재 범위에서 분리된 작업: 저작 구조, 대상 선택 메타데이터, RNG 통합, SO 단일 원본화, Unity 프리팹화
...
P0-A RNG 단일화
    ...
    +---------------------------+
                                v
P0-B 열린 카드 저작 구조 --> P0-C 대상 선택 메타데이터
            |                   |
            +---------+---------+
                      v
              P1-A SO 단일 원본화
...
- `Outcome`, `ConditionTier`, 기본 `CardType`: 표현용 닫힌 enum 분기 유지

이 항목이 실제로 열린 콘텐츠 확장 축으로 바뀔 때만 별도 설계를 거쳐 레지스트리화한다.
