...
- **구조**
  ...
  | 커뮤니티 명명 | 커밋된 매핑(대표 심볼→이름)으로 커뮤니티 이름을 결정론 입힌다 | 클러스터링이 어떻게 계산됐는지 |
...
중요한 것은 게임 로직 아키텍처):
  ...
  - 도구: `source_file`이 `tools/`·`Tools/`로 시작하는 노드 (실측 27개 + 이 계획이 추가하는
    그래프 스크립트 자신) — 게임 로직 아키텍처가 아니다
  - 테스트 프로젝트: `source_file`이 `Tests/`로 시작하는 노드 — `/Tests/` 부분 문자열 규칙이
    저장소 루트의 `Tests/Headless/*.csproj`를 놓쳐 커뮤니티 하나가 새던 것을 2026-08-28
    실측으로 확인해 보강했다
...
**3. 재생성 스크립트** — `Tools/graph/rebuild-graph.sh` (조정자, 로직 없음. 규칙 30).
  ...
  5. `python3 Tools/graph/name_communities.py` — 커뮤니티 이름을 결정론으로 입힌다 (아래
     "커뮤니티 명명" 참조).
  6. 뷰 2장 생성 (아래 4·5) 후, 같은 스크립트의 `--view` 모드로 뷰의 "Community N"
     자리표시자를 그룹 대표 라벨로 치환한다.

  **커뮤니티 명명** — `Tools/graph/name_communities.py` + 커밋되는
  `Tools/graph/community-names.json`(대표 심볼→이름 66개, 2026-08-28 큐레이션).
  graphify 자체의 `.graphify_labels.json`은 커뮤니티 **번호**에 키가 걸려 있어, 규칙 21의
  삭제 후 재생성마다 번호가 재배정되면 이름이 엉뚱한 커뮤니티에 붙는다(실측: 'Manifest'가
  코드 254노드 커뮤니티에). 그래서 이름의 원본을 번호가 아니라 **내용**(커뮤니티 최고 연결
  노드 중 `.`로 시작하지 않는 첫 라벨 = 대표 심볼)에 키를 건 커밋 파일로 두고, 재생성마다
  결정론으로 다시 입힌다. 매핑 미스는 대표 심볼 이름 그대로 남아 자기설명적이며, 그때 매핑에
  항목을 추가하면 된다. LLM 백엔드(claude-cli)는 Claude Code 세션 내 중첩 실행이 막혀 있고
  번호 키 문제도 그대로라 채택하지 않았다(2026-08-28 실측).
