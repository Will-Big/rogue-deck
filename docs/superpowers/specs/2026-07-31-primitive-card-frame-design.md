...
> **2026-09-20 후속 검토:** [B안 프리팹 적용 계획](../plans/2026-09-20-card-b-prefab.md)의
> [HTML 개요](../plans/2026-09-20-card-b-prefab.html)가 새 시각 계약을 제안한다. 승인 후 이 문서의
> §6 기호 문법, §11 카드 배치, §15 관련 시각 단언은 새 계약을 따른다. 코어 대상 의미·설명 합성·
> 반응형 손패 원칙은 유지한다. 기존 비용 돌출·진영색만의 구별 규칙으로 새 목업을 되돌리지 않는다.
...
2. 설명 본문은 카드 전체에서 같은 대상을 가리키는 효과를 한 줄로 모으고 각 줄 앞에 진영 심볼을 붙인다.
...
- 진영: 같은 형태에 아군 `#5DADE2`, 적군 `#E85D5D` 색을 적용
- 방향: 아군은 오른쪽, 적군은 왼쪽이 전열이므로 두 진영의 전열이 카드 가운데를 향함

진영은 색상만으로 구분한다. 채움·윤곽이나 서로 다른 기호를 진영 구분 수단으로 섞지 않는다. 방향 차이는
전열이 전투의 중심을 향하는 위치 의미일 뿐 진영 식별 수단이 아니다. 레일과 대상 도형을 포함한 활성
프리미티브에는 해당 진영색을 적용한다. `자신`은 양쪽 모두 같은 `◎`를 쓰고 색만 다르게 표시한다.
`모두`의 `◇━━◇`는 다른 범위 심볼과 같은 전체 폭을 사용한다.

대상 칸의 콘텐츠는 항상 한 개의 가로 행으로 배치한다.

- 대상 진영이 하나면 해당 심볼 하나를 칸 가운데에 둔다.
- 아군과 적군이 모두 있으면 아군을 왼쪽, 적군을 오른쪽에 나란히 두고 두 심볼 묶음을 가운데 정렬한다.
- 어떤 경우에도 대상 심볼을 세로로 쌓지 않는다.
...
```text
<적군색>◆</적군색> 피해 3. 취약 1.
<아군색>◆</아군색> 방어 3. 소비했다면 방어 4.
```

효과 원본에서 진영이 교차해 등장하더라도 같은 정확한 대상 키의 문장은 카드 전체에서 한 줄로 모은다.
...
위치 범위는 설명 줄에서 반복하지 않는다. 실행 카드에서는 가운데 `SymbolOnlyTargetPanel`의 프리미티브
glyph가 범위를 표시한다. 개입 카드는 구조화된 대상 의미가 있더라도 대상 패널을 만들지 않는다. 대상이
없는 효과만 진영 심볼 없이 본문만 표시한다.

헤드리스 `PlainText`도 모든 유닛 대상 줄에 같은 기본 토큰 `[◆]`를 사용한다. 색을 표현할 수 없으므로
`PlainText`만 읽어서는 진영과 범위를 구분할 수 없다. 이 손실은 의도된 표현 경계다. 진영이나 범위가
필요한 소비처는 평문을 역파싱하지 않고 `CardDescriptionLine.Target`과 `TargetEntries`를 사용한다.
...
- 대상 칸은 `HorizontalLayoutGroup`과 가운데 정렬을 프리팹에서 저작해 0..2개 심볼을 항상 한 행에 둔다.
- 대상이 하나면 가운데, 둘이면 아군 왼쪽·적군 오른쪽인 묶음 전체를 가운데 정렬한다.
- 유닛 대상이 없을 때만 `TargetGlyphView` 대신 가운데 `∅`를 표시한다.
...
- 대상이 없는 경우에도 `∅`를 만들지 않는다. `∅`는 무대상 실행 카드 전용이다.
...
`TargetGlyphView.prefab`은 §6.1의 방향과 길이에 맞게 프리미티브 계층을 갱신한다. 아군과 적군은 같은
도형을 사용하고 직렬화된 진영색만 다르게 적용한다. 자신은 단일 `◎`, 모두는 다른 범위와 같은 전체 폭의
`◇━━◇`로 표시한다. 설명 줄과 대상 glyph가 사용하는 두 직렬화 색은 프리팹 검증에서 서로 같음을
확인한다.
...
- 대상 glyph는 아군의 오른쪽과 적군의 왼쪽이 전열을 향하고, `모두`가 다른 범위와 같은 전체 폭이다.
- 자신은 양 진영 모두 `◎`를 사용하며 아군 `#5DADE2`, 적군 `#E85D5D` 색만 다르다.
- 대상이 하나면 가운데, 둘이면 아군 왼쪽·적군 오른쪽인 한 행으로 가운데 정렬되며 세로 배치는 없다.
