...
심볼 뒤에는 공백 하나만 두며 대괄호는 표시하지 않는다.

- 모든 `Ally` 범위와 `Ally/Self`: 윤곽 마름모 `◇`
- 모든 `Enemy` 범위와 `Enemy/Self`: 채운 마름모 `◆`
- 대상 없음: 접두사 없이 본문만 표시

`◇`와 `◆`은 모양으로 진영을 구분하므로 색상에만 의존하지 않는다. 위치 범위는 설명 줄에서 반복하지
않고 실행 카드 가운데 `SymbolOnlyTargetPanel`의 프리미티브 glyph가 소유한다. 개입 카드와 대상 없는
효과는 진영 심볼을 표시하지 않는다.

헤드리스 `PlainText`에는 가운데 대상 칸이 없으므로 기존 정규 대상 토큰을 유지한다. 대괄호는 이 평문에서
심볼 경계를 나타내는 구두점이며, `Ally/Self`의 `◇◎`와 `Enemy/Self`의 `◎◆`처럼 진영과 범위를 함께
보존한다. 따라서 Unity 설명 블록은 간결한 진영 표식을 사용하면서도 터미널 출력은 대상 정보를 잃지 않는다.
...
표시한다. `DescriptionLineView.Bind`가 `CardDescriptionLine.Target`의 `Faction`만 읽어 `◇ ` 또는
`◆ ` 접두사를 본문과 같은 문자열 흐름에 추가한다. `Range`는 무시하며, 대상이 없으면 접두사를 추가하지
않는다. 중앙 `TargetGlyphView.prefab`과 그 프리미티브 계층은 변경하지 않는다.
...
- 아군·적군 설명은 범위와 무관하게 각각 `◇ `·`◆ ` 한 글자 접두사를 사용한다.
