...
2. 설명 본문은 대상이 달라지는 지점에서 줄을 나누고 각 줄 앞에 대상 심볼을 붙인다.
...
설명 줄은 진영 이름을 텍스트로 쓰지 않는다. 모든 유닛 대상 줄은 `CardTargetKey`에 대응하는 심볼로
시작한다.
...
대괄호는 평문에서 심볼 경계를 나타내는 구두점일 뿐이며 `적`, `아군`, `자신` 같은 진영·범위 단어를
대체하지 않는다. Unity에서는 대괄호 문자 대신 고정 폭 심볼 영역을 사용한다.

- `Ally` 일반 범위: 아군 심볼
- `Enemy` 일반 범위: 적군 심볼
- `Ally/Self`: 아군 방향 표식과 이중 원의 조합
- `Enemy/Self`: 이중 원과 적군 방향 표식의 조합
- 대상 없음: 심볼 영역을 숨기고 텍스트만 표시

헤드리스 `PlainText`도 같은 심볼 토큰을 사용한다. 따라서 Unity가 없어도 네 경우를 단어가 아닌 형태로
구분할 수 있으며, 색상 정보가 없는 터미널에서도 아군 자신과 적 자신이 서로 다른 토큰으로 남는다.
