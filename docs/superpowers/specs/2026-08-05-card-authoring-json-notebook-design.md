...
- 상태: `implemented` — 계획 A(코어)·B(읽기 UI)·C(편집)·D(반영·정리) 완료
...
```text
신규 3 · 수정 5 · 변경 없음 18 · 읽기 오류 1 · 미참조 파일 1
```

"저장소에만 있음"은 성격이 다른 둘로 가른다 — 같은 문구로 묶으면 반대되는 조언을 하게 된다.

- **읽기 오류** — 파싱에 실패해 노트북이 아예 카드로 읽지 못한 파일(§10.2). 고쳐야 내보낼 수 있고,
...
`Statuses/`의 읽기 오류는 이 요약과 내보내기 차단 판정에 들어가지 않는다 — 상태는 노트북이 쓰지
않는 범위 밖 콘텐츠(§15)라 원문을 읽기 전용으로만 보여준다.
