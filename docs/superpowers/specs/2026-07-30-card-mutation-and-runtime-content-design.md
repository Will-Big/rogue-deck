...
**결정 (계획 3d 최종 리뷰, 2026-08-05):** 계획 3d가 지운 필드 단위 골든 — 카드마다 피해·비용·
실행순서·이름을 박아 둔 C# 배열 — 은 되살리지 않는다. 축소판도 만들지 않는다. 이건 누락이 아니라
의도한 선택이다: JSON이 유일 저작 원본인 지금, 26장의 필드를 다시 C# 배열로 옮겨 적으면 이 계획이
막 없앤 이중 원본(§2.3)이 그대로 되살아난다 — 카드 수치를 고치면 JSON과 골든 두 곳을 맞춰야 하는
예전 문제로 돌아간다. `DeckPoolCharacterContentTests`의 `StarterDeckGolden`·
`PartyPrototypeDeckGolden`·`StarterPoolGolden`이 잠그는 카드 id 목록과 덱 구성은 계속 고정한다 —
카드가 뒤바뀌거나 빠지는 회귀는 이것으로 잡히고, 필드값 회귀는 `DescriptionComposerTests`·
`StarterPoolCombatBehaviorTests` 같은 행동 테스트가 개별적으로 잡는다.
