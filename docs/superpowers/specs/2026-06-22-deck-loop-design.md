...
- 문서 유형: `combat-rules`
- 주 도메인: `deck-loop`
- 상태: `current` — 덱·손패·행동 턴 루프 기준

> 현재 카드풀 설계에서 사용하는 전투 사이클과 상태 타이밍은 §3.1을 기준으로 한다. 아래 기존 §3의
> 구현 초안은 데이터 모델과 세션 API 참고용으로 보존하며, 두 문서가 충돌하는 지점은 후속 전투 루프
> 통합 세션에서 정리한다.
...
플레이어 사용 턴 시작
  ↓
플레이어 행동 카드 배치 및 조작 카드 사용
  ↓
플레이어 사용 턴 종료
  ↓
플레이어 및 몬스터 행동 턴 시작
  ↓
플레이어·몬스터 행동 카드 실행 및 해석
  ↓
플레이어 및 몬스터 행동 턴 종료
  ↓
다음 사이클의 드로우
...
`플레이어 및 몬스터 행동 턴 시작`은 플레이어 행동과 몬스터 행동이 각각 따로 시작하는 두 턴이 아니라,
양쪽의 행동 단계가 동시에 시작하는 하나의 공통 단계다. 이 단계 안에서 플레이어와 몬스터의 행동 카드가
실행선 규칙에 따라 해석된다.

#### 단계별 의미

1. **드로우** — 현재 생존 파티원 수와 덱 규칙에 따라 손패를 보충한다.
2. **플레이어 사용 턴 시작** — 플레이어가 이번 사이클에 사용할 카드와 운명력을 확인한다.
3. **플레이어 행동 카드 배치 및 조작 카드 사용** — 행동 카드는 실행선에 배치하고, 조작 카드는 손에서
   즉시 사용해 실행선·대상·카드 규칙 등을 변경한다.
4. **플레이어 사용 턴 종료** — 카드 배치와 조작 입력을 닫고 행동 단계로 넘어간다.
5. **플레이어 및 몬스터 행동 턴 시작** — 양쪽의 행동이 동시에 시작된다.
6. **행동 카드 실행 및 해석** — 플레이어와 몬스터의 카드가 실행순서에 따라 해결되고 이벤트 타임라인을
   생성한다.
7. **플레이어 및 몬스터 행동 턴 종료** — 행동 단계의 모든 카드가 해결된 뒤 종료 훅과 상태 발동을 처리한다.

#### 독 발동 타이밍

`독 X`는 턴 종료가 아니라 **플레이어·몬스터 행동 턴 종료**에 발동한다. 독은 행동 턴 종료 시 대상에게
X만큼 피해를 주고 독 수치를 1 증가시키는 상태다.

행동 턴 종료의 처리 순서는 다음과 같다.

```text
플레이어·몬스터 행동 카드의 모든 실행 완료
  ↓
독 피해 발동
  ↓
독 수치 증가
  ↓
독 피해로 인한 사망·카드 제거·전투 종료 판정
  ↓
다음 사이클
```

플레이어 사용 턴에 배치된 독 카드가 행동 턴에 정상 실행됐다면, 그 행동 턴의 종료에 독이 발동한다.
행동 턴 중 새로 부여된 독도 같은 행동 턴 종료에 발동하는 것을 기본값으로 한다. 행동 중 이미 사망한
대상은 독 발동 대상에서 제외한다.

이 타이밍은 독을 플레이어 사용 턴 직후의 선제 피해로 만들지 않고, 플레이어와 몬스터가 모두 행동한 뒤
발생하는 장기 압박으로 유지한다. `출혈`처럼 피격 즉시 발동하는 상태와도 발동 시점이 겹치지 않는다.
