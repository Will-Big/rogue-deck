...
**목표는 약 102줄 / 2,500토큰이었다.** 못 미친 이유는 셋이고 전부 의도한 것이다.

- **규칙 31 — 12줄 (약 480토큰).** 근거 자체가 강제 장치라 줄이면 규칙이 무력해진다. 3줄로 줄였다면
  약 360토큰을 더 뺐을 것이다.
- **규칙 17 — 5줄.** 착수 후 3줄에서 늘렸다. 아래 「규칙 32개 배치」의 근거를 참고.
- **나머지 30개 규칙의 명령문 자체.** 부피의 원인은 근거였고 그것은 실제로 다 나갔다 — 남은 것은
  규칙 본문이라 더 줄이려면 규칙을 지워야 한다. 그것은 이 작업의 범위가 아니다.

목표 수치는 착수 전 추정이었고, 규칙 17이 5줄로 결정되기 전에 잡은 것이다. **수치를 맞추려고 우회
차단 문장을 잘라내지 않았다.**
...
2. ~~`Tools/rule-note.sh` + 근거 문서 7개~~ — **완료** `0b150bc`
3. ~~lint R5·R7·R-doc~~ / 4. ~~git 훅 4개~~ / 5. ~~Claude 훅 (규칙 15만)~~ — **완료** `eff7345`
6. ~~AGENTS.md 축소~~ / 7. ~~규칙 16 본문 재작성~~ — **완료**

**6번을 먼저 하면 안 됐다.** 근거를 지운 뒤 훅이 준비 안 되어 있으면 그 사이 세션들이 근거 없는
규칙만 받는다. 실제로 R-doc(3단계)이 있어야 6단계에서 잘라낸 규칙 번호를 아직 인용하는 곳이
있는지 기계로 확인할 수 있었다.

**6단계에서 유실을 한 건 잡았다.** 규칙 12의 "Compare 하니스"가 어느 문서에도 없이 사라질 뻔했다.
