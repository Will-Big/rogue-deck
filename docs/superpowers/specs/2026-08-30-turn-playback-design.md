...
- **2026-09-18 후속:** [전투 실행 계약](2026-09-18-combat-execution-contract-design.md) 구현으로 대상을 찾지 못한 효과는 카드를 취소하지 않고 그 효과만 미적용된다
  (`NoValidTarget` 제거, 중간 취소 없음). `CardCancelled`는 차례 전 취소(가로채기)뿐이고 `DamageDealt` 칸도 없다.
  방어 만료는 다음 턴 준비로 옮겨 해석 타임라인에 나오지 않는다(세션 `LastTurnStartTimeline`). 아래 해당 서술은 작성 당시 기준이다.
