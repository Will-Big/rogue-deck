# Starter Pool ScriptableObject Authoring Implementation Record
...
- [x] **Step 1: Write failing CardAsset behavior tests**
...
- [x] **Step 3: Add the minimal serialized fields and mapping**
...
- [x] **Step 1: Write failing pool validation tests**
...
- [x] **Step 3: Implement CardPoolAsset**
...
- [x] **Step 1: Write failing seed behavior tests**
...
- [x] **Step 3: Implement the bootstrap catalog and idempotent seeder**
...
- [x] **Step 3: Implement optional pool export**
...
- [x] **Step 2: Generate `.meta` files and run focused Unity tests**
...
- [x] **Step 4: Archive and index the plan**
...
- 모든 카드와 풀 대상 경로를 쓰기 전에 기존 메인 에셋 타입, `.asset` 확장자, 카드 경로와 ID의 일치를
  검사한다.
- 시더와 export는 공통 검증으로 `starter_pool`, 정확히 22개 참조, 문서의 정확한 ID 집합을 요구한다.
- 생성 도중 예외가 발생하면 이번 실행에서 만든 카드와 풀을 삭제하고 기존 풀의 ID·참조를 복원한다.
- 시더 EditMode 테스트는 임시 부모 폴더까지 비었을 때 삭제해 워크트리에 `.meta`를 남기지 않는다.
- 최종 검증: 헤드리스 397/397, Unity EditMode 469/469 통과.
