...
| 문서 | 상태 | 권위 범위 | 다음 사용 시점 |
|---|---|---|---|
| [graphify 카드 그래프 통합](specs/2026-08-28-graphify-card-graph-design.md) | `current` | 카드·상태 JSON의 그래프 편입, 재생성 단일 명령, 카드·아키텍처 시각화 | 그래프 도구 구현·확장, AGENTS.md 규칙 21·22 개정 |
...
| [전투 타임라인 이벤트 확장](plans/2026-08-28-combat-timeline-event-expansion.md) | `active` | 캐릭터별 HP 변화, 운명력 적립, 상태 소비, 카드 귀속 버프, 대형 이동, 취소 카드 부분 피해의 이벤트화와 이벤트당 Console 로그 |
...
| [graphify 카드 그래프 통합 구현](plans/2026-08-28-graphify-card-graph.md) | `active` | 프루너, 카드 추출기·직접 병합, 뷰 2장, rebuild-graph.sh, AGENTS.md 규칙 21·22 개정 |
...
1. 새 스펙·계획은 **`## 설계 개요`(사람 검수용) + `## 상세`(세션 인계용)** 두 절로 쓴다. 개요가 항상
   최상단이고, 사람은 개요만 읽고 구조를 승인할 수 있어야 한다. 에이전트는 개요를 방향 확인에만 쓰고
   실행 근거는 상세에서 얻는다. 골격과 제약은 `AGENTS.md` 규칙 28·29에 있다. 기존 문서는 소급해
   고치지 않고, 수정할 일이 생겼을 때 이 골격으로 맞춘다.
