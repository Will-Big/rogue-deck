...
- [ ] **[필수] 다중 적 — 적마다 정책과 카드 주인** — 사용자 지정 필수(2026-09-15). 위 항목이 미룬 "정책 API
  ...
  `DeckCombatSession`이 쌍 목록을 받아 적마다 정책을 호출하고 그 적을 카드 주인으로 확정, 실행 순서 보정을
  적별 상태로(`DeckCombatSession.cs:381`의 `Enemies[0].Statuses` 가정 제거), 편성 로더의 "적 정확히 1개"
  검증 삭제. **선행 결정은 2026-09-19에 정해졌다:** 죽은 적은 **정책을 호출하지 않아 카드를 내지 않고**,
  죽은 적이 이미 존에 올린 카드는 **사라진다**(사용자 결정). 시드 결과가 달라지는 것은 허용한다 — 적을
  먼저 죽이는 것도 플레이어의 행동이다(전투 노드 설계 결정 5). 카드 제거는 이미 `DeathProcessor`가
  소유자별로 하고 있으므로(`Assets/Core/Combat/DeathProcessor.cs:79`), 적마다 카드 주인을 확정하면
  그대로 동작한다.
...
- [ ] **[필수] 같은 적 여럿이 한 편성에 나온다** — 사용자 지정 필수(2026-09-15). 전투 안 id 분리가 전투 노드
  설계에서 끝나므로, 다중 적 항목이 닫히면 `["goblin", "goblin"]` 편성이 추가 작업 없이 돌아야 한다.
  확인 테스트만 더한다.
