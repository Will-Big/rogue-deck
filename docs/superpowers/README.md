...
- [ ] **전투 진입 시 적 개체별 기준 수치에 변주를 확정한다 — 2026-09-20 사용자 요구사항.**
  ...
  변동 폭은 기준값의 ±10% 이내를 원칙으로 하되 최소 ±1을 허용한다는 의미로 해석한다
  (사용자 예시: HP 30 → 27~33, 카드 피해 6 → 5~7; 작은 값의 ±1은 10% 제한의 예외).
  정수 반올림 방식, 낮은 기준값에서의 결과 하한, 수치별 추첨 분포·상관관계와 변주 대상의 상세 범위는
