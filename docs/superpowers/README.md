...
| 문서 | 상태 | 범위 |
|---|---|---|
| [HandFan 시각 테스트 씬](plans/2026-09-22-handfan-sandbox.md) — [HTML 검토](plans/2026-09-22-handfan-sandbox.html) | `active` | 최신 master의 HandFanView·HandFanLayoutView를 사용하는 카드 추가·맨 뒤 1장 제거·전체 비우기·장수 표시. codex/handfan-sandbox에서 구현·자동 검증 완료, 사용자 Play 검수·머지 대기. 전투 규칙·새 연출 제외 |
...
테스트를 더한 결과다. 전체 리뷰 이후 최종 수정으로 148 → 150이 됐다. **2026-09-20 실측 161/161**로
더 늘었다), Unity EditMode **672 total / 665 passed / 0 failed / 7 skipped**
...
- [ ] **전투 진입 시 적 개체별 기준 수치에 변주를 확정한다 — 2026-09-20 사용자 요구사항.**
  같은 몬스터라도 개체마다 HP와 사용하는 카드의 피해 수치가 조금씩 다르도록 한다. 원본 몬스터·카드
  콘텐츠에는 기준값만 두고, 전투 진입 시 적 인스턴스를 생성할 때 개체별 수치를 결정해 보관한다.
  이 변주값은 전투 내내 유지하며 턴마다 또는 카드 실행마다 다시 추첨하지 않는다. 같은 카드 정의를
  사용하는 다른 적에게 변주가 전파되지 않도록 원본과 개체별 값을 분리한다.
  변동 폭은 `max(1, ceil(기준값 × 0.1))`로 확정한다(2026-09-20 사용자 추가 결정).
  기준값의 10%를 먼저 올림한 정수 폭을 기준값에서 빼고 더해 양 끝을 포함한 정수 범위를 만든다.
  따라서 올림 때문에 실제 변동률은 10%를 넘을 수 있으며, 엄격한 10% 상한은 적용하지 않는다.
  예시: HP 30 → 27~33, HP 22 → 19~25, 카드 피해 8 → 7~9, 카드 피해 6 → 5~7.
  낮은 기준값에서의 결과 하한, 수치별 추첨 분포·상관관계와 변주 대상의 상세 범위는
  구현 전 설계에서 확정한다. 시드 기반 결정론을 유지하도록 설계한다.
  스테이지 맵 생성 때는 노드의 적 편성을 확정하고, 전투 진입 때는 그 편성에 속한 적들의 개체별
  실수치를 확정하는 두 시점을 구분한다. 의도는 같은 종류의 적 사이에도 차이를 주어 우선 공격 대상의
  판단 여지를 만들고 반복 전투의 단조로움을 줄이는 것이다. 현재 적 생성은 정의의 최대 HP를 그대로
  전달한다(`Assets/Core/Simulation/Run/ContentEncounterSource.cs:31`). 이 항목은 후속 요구사항 기록이며
  지금의 구현 변경 지시는 아니다.

- [ ] **스테이지 맵 생성 시 노드 내용을 미리 확정한다 — 2026-09-20 사용자 요구사항.**
  런 시작 후 해당 스테이지의 맵과 노드들을 표시하기 전에 각 노드의 내용을 결정해 보관한다.
  스테이지별로 등장 가능한 적 편성 풀과 이벤트 풀을 따로 구성한다. 전투 노드에는 특정 적 편성을,
  이벤트 노드에는 특정 이벤트를 배정한다. 시드 기반 추첨을 사용한다면 맵 생성 시 수행하며,
  노드 진입 시에는 다시 추첨하지 않고 이미 배정된 내용을 실행한다.
  현재는 `CombatNode.Begin`에서 편성을 추첨한다
  (`Assets/Core/Simulation/Run/CombatNode.cs:83`, `Assets/Core/Simulation/Run/ContentEncounterSource.cs:23`).
  맵·스테이지 구현 시 이 선택 책임을 맵 생성 단계로 옮긴다. 내부 내용 확정과 맵에서의 정보 공개는
  별개이며, 공개 범위는 추후 설계한다. 이 항목은 후속 요구사항 기록이고 지금의 구현 변경 지시는 아니다.
...
- [x] **[필수] 전투 사이 HP 인계 — 2026-09-19 구현 완료.** 전투를 끝낸 HP가 런에 남고 다음 전투가
  그 HP로 시작한다. `PartyMemberLoadout.Hp`(생략하면 최대 HP)를 세션이 파티원 초기 HP로 쓰고
  (`Assets/Core/Simulation/DeckCombatSession.cs:134`), `CombatNode.Conclude`가 승패와 무관하게
  `RunMember.Hp`에 기록한다(`Assets/Core/Simulation/Run/CombatNode.cs:123`·`146`). 음수 HP는 0으로 깎아
  런에 흘리지 않고, 1~최대 범위를 벗어난 로드아웃 HP는 세션 생성에서 거부한다.

  **선행 결정은 "사망 유지"로 정해졌다**(사용자 결정 2026-09-19). 이긴 전투에서 HP 0이 된 파티원은
  런에서도 죽은 채 남아 `RunState.LivingMembers`에서 빠지므로 다음 전투와 보상 후보에 들어가지 않는다.
  카드는 `RunMember`에 그대로 남는다 — 유산(사망 파티원의 카드 인계)은 아직 설계 밖이다. 체력 1로
  버티는 방식은 **나중에 다른 설계로** 도입한다(아래 「치명타 버티기 재도입」과 같은 줄기).

  같은 커밋에서 `CombatNode.Begin`의 로드아웃이 `RunMember.Cards`를 복사하는지 잠그는 테스트도 더했다.
  헤드리스 740 → 748.

  **남은 결과:** 회복 수단이 아직 없다 — 치유 효과(`EffectKeys`에 없다)도 휴식 노드도 없으므로 런의
  HP는 단조 감소한다. 밸런스를 볼 시점에 회복을 어디에 둘지(카드·보상·노드) 정해야 한다.

- [x] **[필수] 다중 적 — 적마다 정책과 카드 주인 — 2026-09-19 구현 완료.** 세션이 (적, 정책) 쌍
  목록을 받는다(`DeckCombatSession`의 파티 생성자, `Assets/Core/Simulation/EncounterEnemy.cs`).
  매 턴 쌍마다 그 적의 정책을 부르고, 낸 카드의 주인을 그 적으로 확정하며, 실행 순서 보정도 그 적의
  상태로만 한다(`Assets/Core/Simulation/DeckCombatSession.cs:392`). 전투 안 id가 겹치면 생성에서
  거부한다 — 겹치면 카드 주인과 사망 처리가 남의 것을 지목한다.

  **죽은 적은 정책을 부르지 않는다**(사용자 결정 2026-09-19). 카드도 내지 않고 RNG도 소비하지
  않으므로 적을 먼저 죽이면 이후 추첨이 달라진다 — 행동이 다르면 결과가 다른 것이므로 의도다
  (전투 노드 설계 결정 5). 죽은 적이 이미 올린 대기 카드는 `DeathProcessor`가 소유자로 찾아
  없앤다(`Assets/Core/Combat/DeathProcessor.cs:79`).

  `CombatNode.Begin`의 "적 정확히 1개" 예외와 편성 로더의 같은 검증이 사라졌다. 정책 하나만 받는
  옛 경로(플레이어 HP 기반 시뮬레이션)는 첫 번째 적을 행위자로, 나머지를 대상 전용으로 옮겼다 —
  그 서명은 정책이 어느 적 것인지 말하지 않기 때문이다. 헤드리스 748 → 754.

  **편성 저작:** `Content/Battles/goblin_pair.json`(고블린 2마리)을 2026-09-20에 더했다. 편성 후보가
  둘이 되어 노드마다 시드가 하나를 고르므로, 런 시드 1에서는 노드 3이 첫 2마리 전투다(노드 0~2는
  `goblin_single`). **화면 표현은 아직 눈으로 확인되지 않았다** — 적 둘이 나오는 전투의 레이아웃은
  규칙 17대로 사용자 몫이다.

  **편성을 더하면 같은 시드의 결과가 바뀐다.** 후보 목록의 길이·순서가 추첨 입력이기 때문이다
  (전투 노드 설계 「알려진 제약」). `goblin_pair`를 더했을 때 `GoblinParityTests`의 골든 서명이
  실제로 흔들렸고, 그래서 그 테스트는 편성을 `goblin_single`로 고정하도록 고쳤다 — 그 골든이 잠그는
  것은 C#→JSON 이관이지 편성 목록이 아니다. 저장소 콘텐츠를 읽는 다른 테스트 둘도 함께 고쳤다.

- [ ] **보상용 캐릭터 풀의 임시 데이터 교체** — [전투 노드 한 사이클](specs/2026-09-15-combat-node-cycle-design.md)은
  보상을 "살아남은 캐릭터 → 각자의 풀 → 카드"로 구현하지만, 캐릭터가 아직 설계되지 않아 `member_a`·`member_b`
  모두 `starter` 풀을 가리키는 **임시 데이터**다. 캐릭터를 설계하면 캐릭터 JSON의 `pool`만 바꾸고(코드 변경 없음),
  그때 "카드는 정확히 한 풀에만"(카드풀 설계 §1) 검증을 켠다. 같은 시드의 보상 결과는 이때 바뀐다.

- [x] **[필수] 같은 적 여럿이 한 편성에 나온다 — 2026-09-19 확인 완료.** 편성 로더가 `["goblin",
  "goblin", "rat"]`을 그대로 싣고(`BattleContentLoaderTests`), 편성 공급자가 전투 안 id를 `goblin#0`·
  `goblin#1`로 갈라 각자 HP와 카드를 갖는다(`MultiEnemyCombatTests`). 추가 코드는 없었고 확인
  테스트만 더했다. **편성 JSON 저작은 아직 하지 않았다** — 실제로 여러 마리가 나오는 전투를 만들려면
  `Content/Battles/`에 새 파일을 쓰고 화면을 눈으로 확인해야 한다.
...
| 영역 | 상태 | 이유와 재개 기준 |
|---|---|---|
| 런 원 사이클 | `needs-redesign` | 과거 설계가 `재화 없음`, `사망 카드 인계 없음`, 이전 보상 모델을 전제한다. 재개 시 현재 카드풀 문서의 유산·소유권 규칙을 기준으로 새 스펙을 작성한다. **전투 노드 한 판(결과·보상·덱 반영)은 [전투 노드 한 사이클](specs/2026-09-15-combat-node-cycle-design.md)이 맡는다** — 남은 재설계 범위는 맵(노드 목록은 `Map/` 범주)·세이브·영입·유산이다. |
